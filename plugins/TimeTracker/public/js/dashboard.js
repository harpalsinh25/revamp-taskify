$(function () {
    ...
    function loadDashboard() {
        ...
        let comparisonStart = comparisonEnd.clone().subtract(range.endDate.diff(range.startDate, 'days'), 'days');
        let userId = $('#userFilter').val();

        fetchDashboard(start, end, comparisonStart.format('YYYY-MM-DD'), comparisonEnd.format('YYYY-MM-DD'), userId);
    }
    ...
    function fetchDashboard(startDate, endDate, comparisonStartDate, comparisonEndDate, userId) {
        $.ajax({
            ...
            success: function (response) {
                ...
                if ($('#userFilter option').length <= 1) {
                    $('#userFilter').empty().append('<option value="">All team members</option>');
                    employees.forEach(emp => {
                        $('#userFilter').append(`<option value="${emp.id}">${emp.name}</option>`);
                    });
                }
                ...
                renderAverageProductiveHoursPerUserChart(response.data.average_productive_hours_per_user);
    ...
    function renderMetricsCards(metrics, percentage_changes, absolute_changes) {
        ...
        const metricConfig = {
            work_time: { icon: 'bx-time', color: 'primary', title: 'Total Work Time', description: 'Combined active and manual time' },
            active_time: { icon: 'bx-bolt-circle', color: 'success', title: 'Active Time', description: 'Automatically tracked time' },
        ...
        $.each(metrics, function (key, value) {
            ...
            const percentChange = percentage_changes[key] ? percentage_changes[key].display : '';
            const percentDir = percentage_changes[key] ? percentage_changes[key].direction : '';
            const absChange = absolute_changes[key] ? absolute_changes[key].display : '';
            const percentColor = percentDir === 'increase' ? 'text-success' :
                percentDir === 'decrease' ? 'text-danger' : 'text-muted';
            const trendIcon = percentDir === 'increase' ? 'bx-up-arrow-alt' :
                percentDir === 'decrease' ? 'bx-down-arrow-alt' : 'bx-minus';
            const valueDisplay = value.display || '0h 0m';
            value.display = valueDisplay;
            const progressColor = percentDir === 'increase' ? 'success' :
                percentDir === 'decrease' ? 'danger' : 'secondary';
            ...
            const progressPercentValue = Math.min(Math.max(parseFloat(progressPercent) || 0, 0), 100);
    ...
    function renderTopProductiveUsers(topUsers) {
        if (!topUsers.length) {
            $('#topProductiveUsers').html('<div class="text-center text-muted py-4"><i class="bx bx-user-x" style="font-size: 2rem;"></i><p class="mt-2">No user data available</p></div>');
            return;
        }
            ...
            let options = {
                ...
                tooltip: {
                    ...
                    custom: ({ series, seriesIndex, dataPointIndex }) => {
                        const dailyBreakdown = currentData?.data?.current_period?.daily_breakdown || [];
    ...
    function updateTeamStatistics(userData) {
        ...
        const teamAvg = (hours.reduce((a, b) => a + b, 0) / hours.length).toFixed(1);
    ...
    function updateSelectedUsersCount() {
        ...
        if (selectedCount === 0) {
            ...
            $('#selectedUsersCount').text(`${selectedCount} of ${totalUsers} members selected`);
        }
    }

    function renderDailyBreakdownChart(dailyData) {
        if (!dailyData.length) {
            $('#dailyBreakdownChart').html('<div class="text-center text-muted py-4"><i class="bx bx-bar-chart" style="font-size: 2rem;"></i><p class="mt-2">No data available for chart</p></div>');
            return;
        }

        let categories = dailyData.map(item => item.day_name);
        let activeTimes = dailyData.map(item => (item.active_time / 60).toFixed(2));
        let manualTimes = dailyData.map(item => (item.manual_time / 60).toFixed(2));
        let breakTimes = dailyData.map(item => (item.break_time / 60).toFixed(2));
        let idleTimes = dailyData.map(item => (item.idle_time / 60).toFixed(2));
        let manualProcessingTimes = dailyData.map(item => (item.manual_processing_time / 60).toFixed(2));
        ...
        let options = {
            chart: {
                ...
                height: 400,
                stacked: true,
                toolbar: { show: true, tools: { download: true, pan: false, reset: true, zoom: true } },
        ...
        if (dailyChartInstance) {
            ...
            dailyChartInstance = new ApexCharts(document.querySelector("#dailyBreakdownChart"), options);
    ...
    function renderWorkingHoursTrend(dailyData) {
        if (!dailyData.length) {
            $('#workingHoursTrendChart').html('<div class="text-center text-muted py-4"><i class="bx bx-line-chart" style="font-size: 2rem;"></i><p class="mt-2">No data available for chart</p></div>');
            return;
        }

        let categories = dailyData.map(item => item.day_name);
        let workTimes = dailyData.map(item => {
            let total = (item.active_time + item.manual_time + item.break_time + item.idle_time) / 60;
        ...
        let options = {
            chart: {
                ...
                height: 350,
                zoom: { enabled: true },
                toolbar: { show: true, tools: { download: true, pan: true, reset: true, zoom: true } },
                animations: { enabled: true }
            },
        ...
        if (trendChartInstance) {
            ...
            trendChartInstance = new ApexCharts(document.querySelector("#workingHoursTrendChart"), options);
    ...
    function renderAverageProductiveHoursPerUserChart(data) {
        if (!data || !data.length) {
            $('#avgProductiveHoursChart').html('<div class="text-center text-muted py-4"><i class="bx bx-bar-chart-horizontal" style="font-size: 2rem;"></i><p class="mt-2">No user data available</p></div>');
            return;
        }

        let categories = data.map(user => user.name);
        let hours = data.map(user => user.average_productive_hours);
        ...
        let options = {
            ...
            xaxis: {
                categories: categories,
                title: { text: 'Average Hours per Day', style: { fontWeight: 600 } }
            },
            yaxis: {
                labels: {
                    formatter: function (val) {
            ...
            tooltip: {
                y: {
                    formatter: function (val) {
                        let hours = Math.floor(val);
                        let minutes = Math.round((val - hours) * 60);
                        return `${hours}h ${minutes}m average per day`;
                    }
                }
            },
            colors: ['#00E396'],
        ...
        if (avgProductiveChartInstance) {
            ...
            avgProductiveChartInstance = new ApexCharts(document.querySelector("#avgProductiveHoursChart"), options);
    ...
    $(document).on('click', '.top-user-row', function () {
        ...
        $('#userInsightsModalLabel').html(`<i class="bx bx-user-circle me-2"></i>User Insights: ${userName}`);
        $('#modalLoading').show();
        $('#userInsightsContent').hide();

        const modal = new bootstrap.Modal(document.getElementById('userInsightsModal'));
        modal.show();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let workTimes = dailyBreakdown.map(item => {
                    let total = (item.active_time + item.manual_time + item.break_time + item.idle_time + item.manual_processing_time) / 60;
                    return total.toFixed(2);
                });
                ...
                userInsightChartInstance = new ApexCharts(document.querySelector("#userInsightsTrendChart"), options);
                userInsightChartInstance.render();
            },
            error: function () {
                $('#modalLoading').hide();
                $('#userInsightsContent').html('<div class="text-danger text-center py-4"><i class="bx bx-error-circle" style="font-size:2rem;"></i><p class="mt-2">Failed to load user insights. Please try again.</p></div>');
            }
        });
    });
    ...
    function sortUsers(criteria) {
        ...
        switch (criteria) {
            ...
            case 'productive_time':
                ...
                break;
            case 'utilization':
                sortedUsers.sort((a, b) => parseFloat(b.utilization_display) - parseFloat(a.utilization_display));
                break;
            default:
