    ...
    $.ajax({
        ...
        beforeSend: function () {
            $('#attendance-body').html(
                `<tr><td colspan="10" class="text-center text-muted">Loading attendance data...</td></tr>`
            );
        },
        success: function (response) {
            if (response.data && Array.isArray(response.data)) {
                ...
                $('#attendance-body').html(
                    `<tr><td colspan="10" class="text-center text-danger">Invalid data received from server.</td></tr>`
                );
            }
        },
        error: function () {
            $('#attendance-body').html(
                `<tr><td colspan="10" class="text-center text-danger">Failed to load data. Please refresh.</td></tr>`
            );
        }
    });
}
...
function updateTable(data) {
    ...
    if (data.length > 0) {
        ...
        html =
            `<tr><td colspan="10" class="text-center">No attendance records found for the selected filters.</td></tr>`;
    }
