            ...
            <form action="{{url('settings/store_general')}}" class="form-submit-event" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <i class='bx bx-info-circle text-primary' data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_label('total_paid_leaves_info', 'Set the total number of paid leaves each user is entitled to per year.') }}"></i>
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <i class='bx bx-info-circle text-primary' data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_label('leave_accrual_type_info', 'Choose how leaves are allocated: Lump Sum (all at once on Jan 1) or Monthly Accrual (earned monthly).') }}"></i>
                    ...
                    <div class="mb-3 col-md-4">
                        <label for="company_year_start" class="form-label">{{ get_label('company_year_start', 'Company Year Start (MM-DD)') }} <span class="asterisk">*</span></label>
                        <i class='bx bx-info-circle text-primary' data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_label('company_year_start_info', 'Set when your company/fiscal year begins (Month-Day). Leave balances will reset on this date annually. E.g., 01-01 for Jan 1 or 04-01 for Apr 1.') }}"></i>
                        @php
                        ...
                        <small class="text-muted d-block mt-1">Format: MM-DD (e.g., 01-01 for Jan 1, 04-01 for Apr 1)</small>
                    </div>

                    <div class="mb-3 col-md-4">
                        <label for="company_year_end" class="form-label">{{ get_label('company_year_end', 'Company Year End (MM-DD)') }} <span class="asterisk">*</span></label>
                        <i class='bx bx-info-circle text-primary' data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_label('company_year_end_info', 'Set when your company/fiscal year ends (Month-Day). E.g., 12-31 for Dec 31 or 03-31 for Mar 31.') }}"></i>
                        @php
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <div class="mt-2">
                            <button type="button" class="btn btn-sm btn-outline-primary" id="initialize_leave_balances_btn">
                                <i class='bx bx-refresh'></i> {{ get_label('initialize_recalculate_balances', 'Initialize & Recalculate Balances') }}
                            </button>
                            <small class="text-muted d-block mt-1">{{ get_label('initialize_balances_info', 'Creates missing balances and recalculates existing ones based on approved leaves. Safe to run anytime.') }}</small>
                        </div>
                    </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (!response.error) {
                    ...
                    if (response.summary) {
                        var msg = '';
                        if (response.summary.newly_initialized > 0) {
                            msg += 'Newly initialized: ' + response.summary.newly_initialized + ' users. ';
                        }
                        if (response.summary.recalculated > 0) {
                            msg += 'Recalculated: ' + response.summary.recalculated + ' users. ';
                        }
                        msg += 'Total: ' + response.summary.total_users + ' users across ' + response.summary.workspaces + ' workspaces';
                        toastr.info(msg);
                    }
                } else {
