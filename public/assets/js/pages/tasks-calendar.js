class TaskCalendarManager {
    constructor(config = {}) {
        this.config = {
            calendarContainerId: 'taskCalenderDiv',
            ...
            projectId: config.projectId || $('#projectId').val(),
            isFavorites: config.isFavorites || $('#is_favorites').val(),
            ...config
        };
    ...
    async init() {
        if (this.state.isInitialized) {
            console.warn('TaskCalendarManager already initialized');
            return this;
        }
        ...
        try {
            ...
            console.error('Failed to initialize TaskCalendarManager:', error);
            throw error;
        }
    }
    ...
    async loadFilterOptions() {
        ...
        try {
            ...
            this.state.taskStatuses = statusResponse.statuses || statusResponse;
            this.state.taskPriorities = priorityResponse.priorities || priorityResponse;

            this.state.activeFilters.status = this.state.taskStatuses.map(s => s.id.toString());
            this.state.activeFilters.priority = this.state.taskPriorities.map(p => p.id.toString());
    ...
    renderStatusFilters() {
        ...
        const filtersHtml = this.state.taskStatuses.map(status =>
    ...
    renderPriorityFilters() {
        ...
        const filtersHtml = this.state.taskPriorities.map(priority =>
    ...
    bindFilterEvents(type) {
        $(`.${type}-filter`).off('change.tcm').on('change.tcm', (e) => {
    ...
    initializeQuickActions() {
        $('#selectAllTaskFilters').off('click.tcm').on('click.tcm', () => this.selectAllFilters());
        $('#clearAllTaskFilters').off('click.tcm').on('click.tcm', () => this.clearAllFilters());
        $('#refreshTaskCalendar').off('click.tcm').on('click.tcm', () => this.refreshCalendar());
    }
    ...
    updateFilterCounters() {
        ...
        this.state.taskStatuses.forEach(status => {
            $(`#count-status-${status.id}`).text(counts.status[status.id.toString()] || 0);
        });

        this.state.taskPriorities.forEach(priority => {
            $(`#count-priority-${priority.id}`).text(counts.priority[priority.id.toString()] || 0);
        });
    }

    calculateCounts() {
        return this.state.allTasks.reduce((acc, task) => {
            const statusId = task.status_id?.toString();
            const priorityId = task.priority_id?.toString();
    ...
    updateStatistics() {
        ...
        const visibleTasks = this.getVisibleTasksCount();

        $('#total-tasks').text(totalTasks);
        $('#visible-tasks').text(visibleTasks);
        $('#filtered-tasks').text(totalTasks - visibleTasks);
    }

    getVisibleTasksCount() {
        return this.state.allTasks.filter(task => {
            const statusMatch = this.state.activeFilters.status.length === 0 ||
                this.state.activeFilters.status.includes(task.status_id?.toString());
            const priorityMatch = this.state.activeFilters.priority.length === 0 ||
                this.state.activeFilters.priority.includes(task.priority_id?.toString());
    ...
    initializeCalendar() {
        ...
        this.state.calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: (fetchInfo, successCallback, failureCallback) => {
                this.fetchTasks(fetchInfo, successCallback, failureCallback);
            },
            datesSet: () => {
                if (this.state.calendar) {
    ...
    async fetchTasks(fetchInfo, successCallback, failureCallback) {
        try {
            const response = await $.ajax({
                url: this.config.baseUrl + "/tasks/get-calendar-data",
                type: "GET",
                data: {
                    start: fetchInfo.start.toISOString(),
                    end: fetchInfo.end.toISOString(),
                    projectId: this.config.projectId,
                    is_favorites: this.config.isFavorites
                }
            });

            const events = this.transformEvents(response);

            this.state.allTasks = events;
    ...
    handleEventDidMount(info) {
        ...
        const status = this.state.taskStatuses.find(s =>
            s.id.toString() === event.extendedProps.status_id?.toString()
        );
        const priority = this.state.taskPriorities.find(p =>
    ...
    handleDateClick(info) {
        ...
        $('#task_start_date').val(date);
        $('#task_end_date').val(date);
        $('#create_task_offcanvas').offcanvas('show');
    }
    ...
    showUpdateConfirmation(info, type) {
        const modalId = type === 'drag' ? '#confirmDragTaskModal' : '#confirmResizeTaskModal';
        $(modalId).modal("show");

        $(modalId).off("click.tcm", "#confirm").on("click.tcm", "#confirm", () => {
            this.updateTaskDates(info, modalId);
        });

        $(modalId).off("click.tcm", "#cancel").on("click.tcm", "#cancel", () => {
            info.revert();
            $(modalId).modal("hide");
        });
    }

    async updateTaskDates(info, modalId) {
        ...
        try {
            ...
            const response = await $.ajax({
                url: this.config.baseUrl + "/tasks/update-dates",
                type: "PATCH",
                headers: { "X-CSRF-TOKEN": this.config.csrfToken },
                data: {
                    id: info.event.id,
                    start_date: start,
                    due_date: end === "Invalid date" ? start : end,
                }
            });
    ...
    destroy() {
        ...
        $('.status-filter, .priority-filter').off('.tcm');
        $('#selectAllTaskFilters, #clearAllTaskFilters, #refreshTaskCalendar').off('.tcm');
...
async function initializeTaskCalendar(config = {}) {
    try {
        const calendarManager = new TaskCalendarManager(config);
        await calendarManager.init();

        // Store instance globally for external access if needed
        if (!window.taskCalendarInstances) {
            window.taskCalendarInstances = new Map();
        }
        window.taskCalendarInstances.set(config.calendarContainerId || 'taskCalenderDiv', calendarManager);

        return calendarManager;
    } catch (error) {
        console.error('Failed to initialize task calendar:', error);
        throw error;
    }
}
...
$(document).ready(function () {
    const calendarEl = document.getElementById('taskCalenderDiv');
    if (calendarEl) {
        initializeTaskCalendar().catch(console.error);
    }
});
