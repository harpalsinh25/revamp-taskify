...
function IDinfo(id) {
    ...
    if (messenger != 0) {
        ...
        $.ajax({
            url: url + "/idInfo",
            method: "POST",
            data: { _token: csrfToken, id, type_id: $('#chat_type_id').val(), type: $('#chat_type').val() },
            dataType: "JSON",
            success: (data) => {
                if (!data?.fetch) {
                    NProgress.done();
...
function sendMessage() {
    ...
    if (inputValue.length > 0 || hasFile) {
        ...
        formData.append("type_id", $('#chat_type_id').val());
        formData.append("type", $('#chat_type').val());
        $.ajax({
            ...
            beforeSend: () => {
                ...
                if (hasFile) {
                    messagesContainer
                        ...
                        .append(
                            sendTempMessageCard(
                                inputValue + "\n" + loadingSVG("28px", "upload-avatar-loading"),
                                tempID
                            )
                        );
                } else {
...
function getContacts() {
    if (!contactsLoading && !noMoreContacts) {
        ...
        $.ajax({
            url: url + "/getContacts",
            method: "GET",
            data: { _token: csrfToken, page: contactsPage, type_id: $('#chat_type_id').val(), type: $('#chat_type').val() },
            dataType: "JSON",
            success: (data) => {
...
function star(user_id, type_id, type) {
    if (getMessengerId() != auth_id) {
        $.ajax({
            url: url + "/star",
            method: "POST",
            data: { _token: csrfToken, user_id: user_id, type_id: type_id, type: type },
            dataType: "JSON",
            success: (data) => {
                data.status > 0
                    ? $(".add-to-favorite").addClass("favorite")
...
function getFavoritesList() {
    ...
    $.ajax({
        url: url + "/favorites",
        method: "POST",
        data: { _token: csrfToken, type: $('#chat_type').val(), type_id: $('#chat_type_id').val() },
        dataType: "JSON",
        success: (data) => {
            if (data.count > 0) {
...
function getSharedPhotos(user_id) {
    $.ajax({
        url: url + "/shared",
        method: "POST",
        data: { _token: csrfToken, user_id: user_id, type: $('#chat_type').val(), type_id: $('#chat_type_id').val() },
        dataType: "JSON",
        success: (data) => {
...
function messengerSearch(input) {
    ...
    if (!searchLoading && !noMoreDataSearch) {
        ...
        $.ajax({
            url: url + "/search",
            method: "GET",
            data: { _token: csrfToken, input: input, page: searchPage, type_id: $('#chat_type_id').val(), type: $('#chat_type').val() },
            dataType: "JSON",
            success: (data) => {
 ...
 *-------------------------------------------------------------
 */
function deleteConversation(id, typeId, type) {
    $.ajax({
        url: url + "/deleteConversation",
        method: "POST",
        data: { _token: csrfToken, id: id, typeId: typeId, type: type },
        dataType: "JSON",
        beforeSend: () => {
            // hide delete modal
            app_modal({
...
$(document).ready(function () {
    ...
    $("body").on("click", ".messenger-list-item", function () {
        ...
        // Get type and type_id parameters
        const type = getUrlParameter('type');
        const typeId = getUrlParameter('id');

        // Build the URL with or without type and type_id parameters
        let newUrl = `${url}/${userID}`;
        if (type && typeId) {
            newUrl += `?type=${type}&id=${typeId}`;
        }
        routerPush(document.title, newUrl);
    ...
    $(".add-to-favorite").on("click", function () {
        star(getMessengerId(), $('#chat_type_id').val(), $('#chat_type').val());
    });
    ...
    $(".app-modal[data-name=delete]")
        ...
        .on("click", function () {
            const id = $("body")
                ...
                .attr("data-modal");
            if (id == 0) {
                deleteConversation(getMessengerId(), $('#chat_type_id').val(), $('#chat_type').val());
            } else {
                deleteMessage(id);
            }
            app_modal({
